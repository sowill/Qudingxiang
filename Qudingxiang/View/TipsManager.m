...
//    AppDelegate *appdel =  [UIApplication sharedApplication].delegate;
    ...
    rectView.center = CGPointMake(QdxWidth/2, QdxHeight/2 - 40);//向上偏40
...
//    _activityView.frame = Rect((ScreenWidth-20)/2, (ScreenHeight-20)/2, 20, 20);
      _activityView.frame = CGRectMake((QdxWidth-20)/2, (QdxHeight-20)/2, 20, 20);
...
-(void)showLoadingTips:(NSString *)message lockScreen:(BOOL)isLock tag:(int)tag;
{
    ...
    @autoreleasepool {
        ...
        if (isLock) {
            ...
            rectView.center = CGPointMake(QdxWidth/2, QdxHeight/2 - 40);//向上40
        }else{
            //不锁屏
            maskView.frame = CGRectMake(0, 0, 175, 85);
            maskView.center = CGPointMake(QdxWidth/2, QdxHeight/2 - 40);//向上40
